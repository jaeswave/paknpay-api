...
const createLot = async (req, res) => {
  try {
    ...
    const exists = await ParkingLot.findOne({
...
const getLotStats = async (req, res) => {
  try {
    ...
    const [todayStats, weekStats, monthStats, activeSessions, totalSessions] =
      await Promise.all([
        Session.aggregate([
          {
            $match: {
              lotId: new mongoose.Types.ObjectId(lotId),
              status: { $in: ["paid", "cash-paid", "completed"] },
              entryTime: { $gte: today },
            },
          },
          {
        ...
        Session.countDocuments({
          lotId,
          status: { $in: ["pending", "active", "pending-payment"] },
        }),
    ...
    res.json({
      today: {
        ...
        sessions: todayStats[0]?.count || 0,
      },
      week: {
        revenue: weekStats[0]?.revenue || 0,
        sessions: weekStats[0]?.count || 0,
      },
      month: {
        revenue: monthStats[0]?.revenue || 0,
        sessions: monthStats[0]?.count || 0,
      },
      activeSessions,
      totalSessions,
      dailyRevenue,
...
const getLotsForAdmin = async (req, res) => {
  try {
...
const reviewLotApproval = async (req, res) => {
  try {
    ...
    lot.approvedAt = decision === "approved" ? new Date() : null;
    lot.rejectionReason = decision === "rejected" ? rejectionReason || "" : "";
    await lot.save();

    res.json({ message: `Lot ${decision}`, lot });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
